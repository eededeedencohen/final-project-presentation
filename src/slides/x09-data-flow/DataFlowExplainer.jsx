...
const CAPTIONS = [
  ...
  <>עכשיו <em>המתרגם</em> עובד בכיוון ההפוך: המסמכים הגולמיים מהמחסן <em>מתורגמים חזרה לשפה שלנו</em> — כל שורה הופכת לפריט מסודר עם שם ומחיר.</>,
  ...
  <>גם ביציאה אין הנחות: כל פריט חוזר <em>נבדק שוב</em> מול ספר החוקים ומקבל <em>תעודת תקינות</em> — אפשר לסמוך על כל מספר.</>,
  /* 11 — מעבר: בקר האיכות ← שכבת השרת */
  <>ה<em>תשובה</em> המסודרת והבדוקה יוצאת מהשכבה — חזרה אל שכבת השרת שביקשה אותה.</>,
  /* 12 — סיום */
  <>המסלול הושלם: הלוך ושוב, וכל תחנה עשתה את שלה. מה שכבת השרת עושה עכשיו עם התשובה? <em>זה בדיוק השקף הבא</em>.</>,
]
...
export default function DataFlowExplainer({ step = 0 }) {
  return (
    <ExplainerStage kicker="BEHIND THE SCENES · 01" title="שכבת הנתונים — הספרייה של המערכת" step={step} captions={CAPTIONS}>
      ...
      <Pipe path={P.resMooseToModel} lit={step === 9} done={step > 9} packetColor="#e8a33d" packetLabel="מאומתים" />
      <Pipe path={P.resModelToServer} lit={step === 11} done={step > 11} packetColor="#e8a33d" packetLabel="תשובה" />

      {/* עוגן שכבת השרת — כללי, בלי פירוט */}
      <LayerAnchor x={1610} y={110} icon={<Icon name="server" />} label="שכבת השרת" sub="משם מגיעות הבקשות" lit={step === 1 || step >= 11} />
      ...
      <HexStation {...ST.mongoose} logo={mongooseLogo} label="המתרגם" sub="Mongoose · מדבר בשפת המחסן" state={step === 4 || step === 8 ? 'active' : step > 4 ? 'done' : 'idle'} />
      <HexStation {...ST.model} icon={<Icon name="shield" size={64} className="x09-ic" />} label="בקר האיכות" sub="Model · שום נתון פגום לא עובר" state={step === 2 || step === 10 ? 'active' : step > 2 ? 'done' : 'idle'} />
      ...
      <Callout x={485} y={405} w={300} show={step === 8}>
        הפעם הפוך: כל מסמך גולמי מהמחסן הופך ל<strong>פריט מסודר</strong> — שם, רשת ומחיר.
      </Callout>
