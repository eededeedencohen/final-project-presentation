...
/* ============================================================
   ...
   שלב אי-זוגי = חבילה אחת נוסעת בצינור אחד;
   שלב זוגי = תחנה אחת נדלקת עם כרטיס ההסבר שלה.
   שלב 11 = מסע חזרה אחד רצוף על נתיב אחד ארוך.
   ============================================================ */
...
const CAPTIONS = [
  ...
  /* 10 */ <>ה<em>מחירים חזרו</em> — עכשיו המנהל בונה את טבלת ההשוואה.</>,
  /* 11 */ <>ה<em>תשובה המוכנה</em> יוצאת החוצה — כל התחנות רק מעבירות אותה, <em>העבודה כבר נעשתה</em>.</>,
  /* 12 */ <><em>מהקליק ועד התשובה — פחות משנייה</em>. ומה המסך עושה עם זה? זה בדיוק השקף הבא.</>,
]
...
/* צינורות: קשת עליונה = הבקשה (ימין→שמאל), צעד אחד בכל שלב.
   returnJourney = נתיב אחד ארוך ורצוף (שמאל→ימין), עובר מאחורי
   התחנות — חבילה אחת עושה עליו את כל דרך החזרה. */
const P = {
  ...
  returnJourney:
    'M 536 276 C 585 324, 680 324, 734 276 L 876 276 C 925 324, 1020 324, 1074 276 L 1216 276 C 1265 324, 1360 324, 1414 276 L 1558 276 C 1602 326, 1652 316, 1692 262',
}
...
export default function ServerFlowExplainer({ step = 0 }) {
  return (
    <ExplainerStage kicker="BEHIND THE SCENES · 02" title="שכבת השרת — מרכז התיאום" step={step} captions={CAPTIONS}>
      ...
      {/* מסע החזרה: חבילה אחת על נתיב אחד רצוף, מהמנהל ועד המסך */}
      <div className="x11-return">
        <Pipe path={P.returnJourney} lit={step === 11} done={step > 11} packetColor="#e8a33d" packetLabel="תשובה" />
      </div>

      {/* עוגני השכבות השכנות */}
      <LayerAnchor x={40} y={110} icon={<Icon name="database" />} label="שכבת הנתונים" sub="המחסן שראינו קודם" lit={step === 0 || step === 9 || step === 10} />
      <LayerAnchor x={1680} y={110} icon={<Icon name="layout" />} label="שכבת הקליינט" sub="המסך של המשתמש" lit={step === 0 || step === 1 || step === 11 || step === 12} />

      {/* תחנות שכבת השרת — כל תחנה נדלקת בשלב ההסבר שלה בלבד */}
      <HexStation {...ST.manager} size={130} icon={<Icon name="user" size={56} className="x11-ic" />} label="המנהל" sub="Controller · מקבל ההחלטות" state={stateOf(step, 8)} />
      <HexStation {...ST.desk} size={130} icon={<Icon name="route" size={56} className="x11-ic" />} label="המוקדן" sub="Routes · מנתב כל פנייה" state={stateOf(step, 6)} />
      <HexStation {...ST.guard} size={130} logo={expressLogo} label="שומר הסף" sub="Express · הדלת הראשית" state={stateOf(step, 4)} />
      <HexStation {...ST.engine} size={130} logo={nodejsLogo} label="המנוע" sub="Node.js · מריץ את הכול" state={stateOf(step, 2)} />
      ...
      {/* שלב 12: סיום — התוצאה ליד המסך + ניצוצות */}
      <InfoCard x={1520} y={420} w={340} title="מה חזר למסך?" show={step === 12} items={[
        '🏆 רמי לוי · ₪142 — הסל הזול ביותר',
        'שופרסל · ₪151',
        'ויקטורי · ₪159',
      ]} />
      <SparkBurst x={1775} y={185} show={step === 12} />
    </ExplainerStage>
  )
}
