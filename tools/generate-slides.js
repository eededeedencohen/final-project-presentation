...
const FONT_MAP = {
  "Open Sans Bold": {
    fam: "'Open Sans', 'Heebo Variable', sans-serif",
    w: 700,
  },
  ...
  "Liberation Sans": {
    fam: "Arial, 'Liberation Sans', 'Heebo Variable', sans-serif",
    w: 400,
  },
  ...
  "Montserrat Semi-Bold": {
    fam: "'Montserrat', 'Heebo Variable', sans-serif",
    w: 600,
  },
  "Montserrat Semi-Bold Italics": {
    fam: "'Montserrat', 'Heebo Variable', sans-serif",
    w: 600,
    i: true,
  },
  "League Spartan": {
    fam: "'League Spartan', 'Heebo Variable', sans-serif",
    w: 400,
  },
  "Helvetica World Bold": {
    fam: "Arial, Helvetica, 'Heebo Variable', sans-serif",
    w: 700,
  },
...
const LABELS = {
  ...
  13: "Client/SPA",
  14: "Client Flow",
  15: "פיצ׳רים - תפעול קנייה",
  16: "פיצ׳רים - AI",
  17: "אתגרים",
  18: "שיפורים פוטנציאלים",
  19: "מסקנות והשפעה",
  20: "תודה",
};
...
const TEXT_BREAKS = {
  ...
  6: [
    ...
    {
      find: "אלגוריתם סיבוכיות נמוכה להשוואת עגלות",
      to: "אלגוריתם סיבוכיות\nנמוכה להשוואת\nעגלות",
    },
...
function genSlide(slide) {
  ...
  const runClass = (r, textScale) => {
    ...
    if (!runClassMap.has(key)) {
      ...
      if (r.spc)
        lines.push(
          `  letter-spacing: ${Math.round(r.spc * PT * 100) / 100}px;`,
        );
  ...
  const emitText = (el, cls, pad) => {
    ...
    slideParas: for (const [pi, para] of el.text.paras.entries()) {
      const align =
        { l: "left", ctr: "center", r: "right", just: "justify" }[para.align] ||
        "left";
      const lh =
        typeof para.lineSpacing === "number" ? para.lineSpacing * 1.22 : null;
      const styleBits = [`textAlign: '${align}'`];
      if (lh) styleBits.push(`lineHeight: ${Math.round(lh * 100) / 100}`);
      if (para.spcBef)
        styleBits.push(
          `marginTop: '${Math.round(para.spcBef * PT * 100) / 100}px'`,
        );
      ...
      for (const rule of TEXT_BREAKS[slide.num] || []) {
        for (const r of para.runs) {
          if (r.text && r.text.includes(rule.find))
            r.text = r.text.replace(rule.find, rule.to);
        }
      }
      ...
      if (!hasText) {
        const sz =
          Math.round((para.endSizePt || 18) * PT * (el.textScale || 1) * 100) /
          100;
        out.push(
          `${pad}  <p className="pp" dir="auto" style={{ ${styleBits.join(", ")}, fontSize: '${sz}px' }}>{'\\u00A0'}</p>`,
        );
        continue slideParas;
      }
      const spans = para.runs
        ...
        .join("");
      out.push(
        `${pad}  <p className="pp" dir="auto" style={{ ${styleBits.join(", ")} }}>${spans}</p>`,
      );
    }
    out.push(`${pad}</div>`);
    return out;
  ...
  const emitEl = (el, depth, parent) => {
    ...
    if (el.kind === "group") {
      ...
      for (const child of el.children)
        out.push(...emitEl(child, depth + 1, cls));
      out.push(`${pad}</div>`);
      return out;
    }
    ...
    if (el.kind === "pic") {
      ...
      if (el.clipPath && !isFullRect(el.clipPath)) {
        const cp = scalePathD(
          el.clipPath.d,
          el.w / el.clipPath.vbW,
          el.h / el.clipPath.vbH,
        );
      ...
      if (el.fillRect) {
        ...
        out.push(
          `${pad}<div className="${cls} pel"><img src={${v}} alt="" /></div>`,
        );
      } else {
        css.push(`.${cls} {\n${style.join("\n")}\n  object-fit: fill;\n}`);
    ...
    if (el.kind === "svg") {
      ...
      if (el.gradient) {
        ...
        const stops = el.gradient.stops
          .map(
            (s) =>
              `<stop offset="${Math.round(s.pos * 1000) / 10}%" stopColor="${s.color}" stopOpacity="${s.alpha}" />`,
          )
          .join("");
    ...
    if (el.gradient) {
      ...
      const stops = el.gradient.stops
        .map(
          (s) =>
            `${rgba({ color: s.color, alpha: s.alpha })} ${Math.round(s.pos * 1000) / 10}%`,
        )
        .join(", ");
    ...
    if (el.stroke)
      style.push(
        `  border: ${Math.max(el.stroke.w, 1)}px solid ${rgba(el.stroke)};`,
      );
    css.push(`.${cls} {\n${style.join("\n")}\n}`);
    out.push(`${pad}<div className="${cls} pel">`);
  ...
  for (const [i, el] of slide.elements.entries()) {
    ...
    lines[0] = lines[0].replace(
      'className="',
      `style={{ '--d': ${d} }} className="fx fx-fade `,
    );
    body.push(...lines);
  }
...
`;
  ...
  fs.writeFileSync(
    path.join(dir, `Slide${String(slide.num).padStart(2, "0")}.jsx`),
    comp,
  );
  fs.writeFileSync(path.join(dir, "styles.css"), cssOut);
  return {
    id,
    comp: `Slide${String(slide.num).padStart(2, "0")}`,
    label: LABELS[slide.num] || `שקף ${slide.num}`,
  };
}
