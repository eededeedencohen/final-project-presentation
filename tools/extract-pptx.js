...
function readCustGeom(custGeom, extW, extH) {
  ...
  for (const p of findChildren(pathLst, "a:path")) {
    ...
    for (const cmd of kids(p)) {
      const t = tagOf(cmd);
      const pts = findChildren(cmd, "a:pt").map(
        (pt) => `${attrs(pt).x} ${attrs(pt).y}`,
      );
...
function shapeToElement(node, ctx, rels, warnings) {
  ...
  const blipFill =
    findChild(spPr, "a:blipFill") || findChild(node, "p:blipFill");
  if (blipFill) {
...
for (const f of files) {
  ...
  console.log(
    `slide ${num}: ${flat.length} shapes - ` +
      ["pic", "svg", "box", "line"]
        .map((k) => `${k}:${flat.filter((x) => x === k).length}`)
        .join(" ") +
